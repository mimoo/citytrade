...
import { Blockchain, get_provider, check_network } from "./eth_provider";
...
export class CityTrade {
    ...
    async init() {
        ...
        if (this.blockchain.provider != null) {
            this.contract = new ethers.Contract(this.address, this.abi, this.blockchain.provider);
    ...
    async get_cities() {
        ...
        let filter_cities = this.contract.filters.NewCity();
        let cities_resp = await this.contract.queryFilter(filter_cities);
    ...
    async get_city(city_id) {
        ...
        let info = await this.contract.cities(city_id)

        let owner = info.owner;
        let owner_ens = await ens_name(this.provider, owner);
    ...
    async get_offers() {
        ...
        let filter_offers = this.contract.filters.OfferForCity();
        let offers = await this.contract.queryFilter(filter_offers);
        console.log("offers:", offers);

        // get all the cancelled offers
        // event CancelOfferForCity(uint256 offerId);
        let filter_offers_cancelled = this.contract.filters.CancelOfferForCity();
        let offers_cancelled = await this.contract.queryFilter(filter_offers_cancelled);
        ...
        let filter_city_sold = this.contract.filters.CitySold();
        let city_sold = await this.contract.queryFilter(filter_city_sold);
        console.log("city_sold:", city_sold);

        // format
        city_sold = await Promise.all(city_sold, async (offer) => {
            let block_number = offer.blockNumber;
            let block = await get_block(this.provider, block_number);
        ...
        for (let offer of pending_offers) {
            ...
            let from_ens = await this.blockchain.ens_name(from);
...
//
// Sunset this stuff once the class is done ^
//
